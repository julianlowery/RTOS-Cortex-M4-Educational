...
void rtos_init() {
    ...
    NVIC_SetPriority(SysTick_IRQn,
                     0xFE);  // just above PendSV (we don't want Pend_SV to nest inside sysTick)
...
void task_create(rtosTaskFunc_t function_pointer, void* function_arg, priority_t task_priority) {
    ...
    for (uint8_t count = 0; count < 4; count++) push_to_stack(&tcb_array[task_number], 0x1);
    push_to_stack(&tcb_array[task_number], (uint32_t)function_arg);
    for (uint8_t count = 0; count < 8; count++) push_to_stack(&tcb_array[task_number], 0x1);
...
void semaphore_take(semaphore_t* sem) {
    ...
    if (sem->count > 0) {
        ...
        __ISB();  // flush CPU pipeline, guarantee new instruction fetch accounts for the updated
                  // hardware state
    }
